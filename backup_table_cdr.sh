...
mysql -u admin -e "select GROUP_CONCAT(CONCAT(\"\",COLUMN_NAME,\"\")) as col_names from INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'cdr' AND TABLE_SCHEMA ='asteriskcdrdb' ORDER BY ORDINAL_POSITION" | grep -v col_names | sed 's/,/","/g' | sed -E 's/^|$/"/g' > $backup_dir"/"$backup_file"_header"

mysql -u admin asteriskcdrdb -e "SELECT * from cdr where DATE_FORMAT(calldate , '%Y-%m') = '"$(date -d "-1 months" +%Y-%m)"' INTO OUTFILE  '"$backup_dir"/"$backup_file"_data' FIELDS TERMINATED BY ','  ENCLOSED BY '\"' LINES TERMINATED BY '\n'"
